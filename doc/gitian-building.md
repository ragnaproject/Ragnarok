...
*Setup instructions for a gitian build of Ragnarok using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Ragnarok
...
- [Building Ragnarok](#building-ragnarok)
...
Clone the git repositories for ragnarok and gitian and then checkout the ragnarok version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the ragnarok repository
...
To build Ragnarok (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the ragnarok repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ragnarok/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/ragnaproject/Ragnarok
...
For example:
```bash
URL=https://github.com/ragnaproject/Ragnarok.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit ragnarok=${COMMIT} --url ragnarok=${URL} ../Ragnarok/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ragnarok=${COMMIT} --url ragnarok=${URL} ../Ragnarok/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ragnarok=${COMMIT} --url ragnarok=${URL} ../Ragnarok/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ragnarok-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ragnarok-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/ragnarok-build.assert
```
...
[ragnaproject/gitian.sigs](https://github.com/ragnaproject/gitian.sigs/) repository, or if that's not possible to create a pull
request.
