...
#include "ui_zragnarokcontroldialog.h"
...
std::list<std::string> ZRagnarokControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZRagnarokControlDialog::listMints;

ZRagnarokControlDialog::ZRagnarokControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZRagnarokControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZRagnarokControlDialog::~ZRagnarokControlDialog()
{
    delete ui;
}

void ZRagnarokControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZRagnarokControlDialog::updateList()
{
...
void ZRagnarokControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZRagnarokControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZRagnarok_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZRagnarokControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZRagnarokControlDialog::GetSelectedMints()
{
...
void ZRagnarokControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
