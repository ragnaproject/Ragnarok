...
#include "zragnarokcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zRAGNA ought to be enough for anybody." - Bill Gates, 2017
    ui->zRAGNApayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzRAGNASyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzRAGNA->setEnabled(false);
        ui->pushButtonMintzRAGNA->setToolTip(tr("zRAGNA is currently disabled due to maintenance."));

        ui->pushButtonSpendzRAGNA->setEnabled(false);
        ui->pushButtonSpendzRAGNA->setToolTip(tr("zRAGNA is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zRAGNApayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzRAGNA_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zRAGNA is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zRAGNA...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zRAGNA in ") +
...
void PrivacyDialog::on_pushButtonSpendzRAGNA_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zRAGNA is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zRAGNA
        sendzRAGNA();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zRAGNA
    sendzRAGNA();
}

void PrivacyDialog::on_pushButtonZRagnarokControl_clicked()
{
    ZRagnarokControlDialog* zRAGNAControl = new ZRagnarokControlDialog(this);
    zRAGNAControl->setModel(walletModel);
    zRAGNAControl->exec();
}

void PrivacyDialog::setZRagnarokControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzRAGNASelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzRAGNA()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Ragnarok Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zRAGNApayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zRAGNA is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " RAGNAROK </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zRAGNA</b>";
    ...
    // use mints from zRAGNA selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZRagnarokControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZRagnarokControlDialog::GetSelectedMints();
    }
    ...
    // Clear zragnarok selector in case it was used
    ZRagnarokControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zRAGNA Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Ragnarok, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zRAGNA Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zRAGNA </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zRAGNA "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zRAGNA "));
    ui->labelzRAGNAAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzRAGNASyncStatus->setVisible(fShow);
}
